...
inline std::vector<std::size_t> laplacian_neighborhood(const SparseMat &laplacian,
                                                       const std::vector<Float> &stationary_sqrt,
                                                       std::size_t initial, std::size_t capacity) {
    const std::size_t n_rows = laplacian.n_rows();
    if (initial >= n_rows || capacity == 0 || capacity > n_rows) {
    ...
    // Compare only the priority weight; insertion breaks ties as in the old indexed heap.
    const auto higher_priority = [](const FrontierEntry &a, const FrontierEntry &b) {
        if (a.weight != b.weight)
            return a.weight < b.weight;
        return a.insertion > b.insertion;
    };
    std::priority_queue<FrontierEntry, std::vector<FrontierEntry>, decltype(higher_priority)>
        frontier(higher_priority);
    ...
    const auto expose = [&](std::size_t state) {
        ...
        for (auto k = row_begin; k < row_end; ++k) {
            ...
            if (neighbor != state && !selected[neighbor] && !in_frontier[neighbor]) {
                ...
                frontier.push({stationary_sqrt[neighbor], insertion++, neighbor});
            }
        }
    };
    ...
    while (states.size() < capacity) {
        ...
        const std::size_t state = frontier.top().state;
        frontier.pop();
        if (!selected[state]) {
            selected[state] = true;
...
template <typename SparseMat, typename Float = double, typename State = std::vector<int>>
inline Subnetwork<Float, std::size_t, State>
laplacian_subnetwork(const SparseMat &laplacian, const std::vector<Float> &stationary_sqrt,
    ...
    auto generator = SparseMatrix<Float, std::size_t>::from_triplets(n, n, rows, columns, values);
...
laplacian_restrictions(const SparseMat &laplacian, const std::vector<Float> &stationary_sqrt,
                       std::size_t initial, std::size_t capacity, int count) {
    if (count < 1) {
    ...
    for (int step = 0; step < count; ++step) {
        subnetworks.push_back(laplacian_subnetwork<SparseMat, Float, State>(
...
laplacian_else_trajectory(const ReversibleLaplacianType &generator, Index initial,
    ...
    for (; completed_steps < options.maximum_steps && time < final_time; ++completed_steps) {
        ...
        const auto position = subnetwork.find(State{static_cast<int>(current)});
        if (position < subnetwork.size())
            source(position, 0) = static_cast<Float>(1);
