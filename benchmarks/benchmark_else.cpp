...
int main() {
    ...
    for (std::size_t j = 0; j < n; ++j) {
        ...
        // Birth: j -> j + 1
        if (j + 1 < n) {
            rows.push_back(j + 1); cols.push_back(j); vals.push_back(birth_rate);
            col_sum += birth_rate;
        } else {
        ...
        if (j > 0) {
            ...
            rows.push_back(j - 1); cols.push_back(j); vals.push_back(d);
            col_sum += d;
        }
        // Diagonal: -col_sum
    ...
    else_sim::Subnetwork<double, std::size_t, std::vector<int>> subnetwork(
    ...
    for (std::size_t k : {1, 2, 4, 8, 12, 16, 20}) {
        ...
        for (int r = 0; r < 200; ++r) { volatile double l = subnetwork.cut_time_loss(u, block); (void)l; }
        auto t1 = std::chrono::high_resolution_clock::now();
        double w_us = std::chrono::duration<double, std::micro>(t1 - t0).count() / 200.0;

        auto t2 = std::chrono::high_resolution_clock::now();
        for (int r = 0; r < 200; ++r) { volatile double l = subnetwork.naive_cut_time_loss(u, block); (void)l; }
