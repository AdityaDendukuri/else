...
ToggleSystem make_toggle(double scale, markovkit::State start, double u_basin, double v_basin, double plot_max) {
...
int main() {
    ...
    std::cout << "1. Simulating Regime 1: High Barrier (scale = 0.20, " << n_paths << " trajectories)...\n";
    ...
    for (int seed = 1; ssa1_u.size() < n_paths && seed <= 100; ++seed) {
        auto traj = ssa::gillespie(sys1.model, sys1.rates, sys1.initial_state, 0.0, final_time, seed, max_steps);
        int max_v = 0;
        for (const auto &st : traj.states) max_v = std::max(max_v, st[1]);
    ...
    else_sim::ELSEOptions opt1{.capacity = 600, .expansion_depth = 1, .tolerance = 1e-12, .maximum_steps = max_steps};
    auto ens1 = else_sim::else_ensemble(sys1.model, sys1.rates, sys1.initial_state, 50, 0.0, final_time, opt1, 8);
    ...
    std::cout << "2. Simulating Regime 2: Lower Barrier (scale = 0.08, " << n_paths << " trajectories)...\n";
    ...
    for (std::size_t i = 0; i < n_paths; ++i) {
        auto traj = ssa::gillespie(sys2.model, sys2.rates, sys2.initial_state, 0.0, final_time, static_cast<int>(10 + i), max_steps);
        for (const auto &st : traj.states) {
    ...
    // Shared ELSE ensemble
    else_sim::ELSEOptions opt2{.capacity = 250, .expansion_depth = 1, .tolerance = 1e-12, .maximum_steps = max_steps};
    auto ens2 = else_sim::else_ensemble(sys2.model, sys2.rates, sys2.initial_state, n_paths, 0.0, final_time, opt2, 42);
    ...
    num::plt::plot(sys1.start_x, sys1.start_y, "Start (17, 1)", "points pt 9 ps 1.8 lc rgb '#000000'");
    num::plt::plot(sys1.basin_u_x, sys1.basin_u_y, "U-high basin", "points pt 7 ps 2.0 lc rgb '#2980b9'");
    num::plt::plot(sys1.basin_v_x, sys1.basin_v_y, "V-high basin (unreached)", "points pt 7 ps 2.0 lc rgb '#d35400'");
    ...
    num::plt::plot(sys1.start_x, sys1.start_y, "Start (17, 1)", "points pt 9 ps 1.8 lc rgb '#000000'");
    num::plt::plot(sys1.basin_u_x, sys1.basin_u_y, "U-high basin", "points pt 7 ps 2.0 lc rgb '#2980b9'");
    num::plt::plot(sys1.basin_v_x, sys1.basin_v_y, "V-high basin (found)", "points pt 7 ps 2.0 lc rgb '#d35400'");
    ...
    num::plt::plot(sys2.start_x, sys2.start_y, "Start (7, 1)", "points pt 9 ps 1.8 lc rgb '#000000'");
    num::plt::plot(sys2.basin_u_x, sys2.basin_u_y, "U-high basin", "points pt 7 ps 2.0 lc rgb '#2980b9'");
    num::plt::plot(sys2.basin_v_x, sys2.basin_v_y, "V-high basin", "points pt 7 ps 2.0 lc rgb '#d35400'");
    ...
    for (std::size_t m : test_counts) {
        ...
        auto ens = else_sim::else_ensemble(sys2.model, sys2.rates, sys2.initial_state, m, 0.0, 15.0,
                                           {.capacity = cap, .maximum_steps = 15}, 42);
        ...
        double frac = std::min(1.0, static_cast<double>(entrances.size()) / std::max(1.0, static_cast<double>(m)));

        traj_counts_dbl.push_back(static_cast<double>(m));
        sharing_fractions.push_back(frac);
        std::cout << "  Trajectories: " << std::setw(6) << m
                  << " | Distinct Entrances / Trajectories: " << std::fixed << std::setprecision(4) << frac << "\n";
    }

    num::plt::subplot(1, 1);
    num::plt::plot(traj_counts_dbl, sharing_fractions, "Distinct entrances / Trajectories", "linespoints pt 7 lw 2 lc rgb '#2980b9'");
