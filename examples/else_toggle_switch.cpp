...
struct ToggleSystem {
    ...
    num::array<double> basin_u_x;
    num::array<double> basin_u_y;
    num::array<double> basin_v_x;
    num::array<double> basin_v_y;
    num::array<double> start_x;
    num::array<double> start_y;
    double plot_max;
};
...
int main() {
    ...
    constexpr num::idx max_steps = 100000;
    constexpr num::idx n_paths = 15;
    ...
    // Independent SSA trajectories for Regime 1 (trapped in U-high basin)
    num::array<num::array<double>> ssa1_u, ssa1_v;
    ...
    num::array<num::array<double>> else1_u(n_paths), else1_v(n_paths);
    for (num::idx i = 0; i < n_paths && i < ens1.size(); ++i) {
    ...
    // Independent SSA trajectories
    num::array<num::array<double>> ssa2_u(n_paths), ssa2_v(n_paths);
    for (num::idx i = 0; i < n_paths; ++i) {
    ...
    num::array<num::array<double>> else2_u(n_paths), else2_v(n_paths);
    for (num::idx i = 0; i < n_paths && i < ens2.size(); ++i) {
    ...
    for (num::idx i = 0; i < n_paths; ++i) {
        std::string label = (i == 0) ? "SSA trajectories (15)" : "";
    ...
    num::plt::title("High Barrier (s=0.20): SSA (Trapped in U-basin)");
    num::plt::xlabel("U");
    num::plt::ylabel("V");
    ...
    for (num::idx i = 0; i < n_paths; ++i) {
        std::string label = (i == 0) ? "Labeled subsweep trajectories (15)" : "";
    ...
    num::plt::plot(sys1.basin_v_x, sys1.basin_v_y, "V-high basin (found)",
                   "points pt 7 ps 2.0 lc rgb '#d35400'");
    num::plt::title("High Barrier (s=0.20): Labeled subsweep (Escapes to V-basin)");
    num::plt::xlabel("U");
    num::plt::ylabel("V");
    num::plt::xlim(0.0, sys1.plot_max);
    num::plt::ylim(0.0, sys1.plot_max);
    ...
    num::plt::plot(sys2.basin_v_x, sys2.basin_v_y, "V-high basin",
                   "points pt 7 ps 2.0 lc rgb '#d35400'");
    num::plt::title("Low Barrier (s=0.08): SSA (Transitions)");
    num::plt::xlabel("U");
    num::plt::ylabel("V");
    ...
    num::plt::plot(sys2.basin_v_x, sys2.basin_v_y, "V-high basin",
                   "points pt 7 ps 2.0 lc rgb '#d35400'");
    num::plt::title("Low Barrier (s=0.08): Labeled subsweep (Transitions)");
    num::plt::xlabel("U");
    num::plt::ylabel("V");
    num::plt::xlim(0.0, sys2.plot_max);
    num::plt::ylim(0.0, sys2.plot_max);
    ...
    const num::array<num::idx> test_counts = {10, 20, 50, 100, 250, 500, 1000};
    num::array<double> traj_counts_dbl;
    num::array<double> sharing_fractions;

    for (num::idx m : test_counts) {
        ...
        num::key_set<markovkit::State> entrances;
